...
# The integrand envelope decays as exp(-0.5*v*tau*w^2), so as tau -> 0 it stays wide and the
# integral has to be carried further out. UMAX = 2500 is ample away from expiry but truncates
# while the integrand is still ~8% of peak in the last few bars, which distorts the time Greeks
# there (tconv by ~55% in the final bar). UMAX = 10000 is converged: identical to 20000 and 40000
# to every digit printed. NPOINTS scales with it to hold the step dw fixed at the old value.
NPOINTS = 8192    # number of points for the integral in the pricer
UMAX = 10000.0    # upper end of the integral (0DTE needs a wide range)
...
def pricer_validation():
    ...
    # xi = 1e-4, not smaller: C and D both divide by xi^2, so driving xi to ~1e-8
    # cancels a vanishing numerator against a 1e-16 denominator and the limit test
    # reports its own round-off (~1e-3) as a pricing error. 1e-4 lands at ~1e-10.
    x0, r0 = XI, RHO
    XI, RHO = 1e-4, 0.0
    ...
    # 4. Integration-limit convergence in the final bar -- the claim the paper
    #    actually leans on. Away from expiry UMAX is slack; as tau -> 0 the
    #    envelope exp(-v tau w^2 / 2) stays wide and the limit, not the point
    #    count, is what binds. dw is held fixed so only the limit varies, and
    #    the time Greeks are printed because they are the ones truncation hits.
    #    Truncation error depends on v*tau, so this one runs at the sample's own
    #    variance level rather than the module default -- that is the regime the
    #    paper's claim is about.
    u0, n0 = UMAX, NPOINTS
    _, _, _v = load_days(find_data_file())
    v_bar = float(np.mean(np.concatenate(_v)))
    tau_bar = ONE_DAY / 77.0                      # one five-minute bar to expiry
    dw = u0 / n0
    print(f"  integration-limit convergence (final 5-min bar, v={v_bar:.4f}, dw held fixed):")
    print(f"    envelope exp(-v*tau*w^2/2) at w=2500: "
          f"{100 * math.exp(-0.5 * v_bar * tau_bar * 2500 ** 2):.1f}% of peak")
    for u in [2500.0, 10000.0, 20000.0, 40000.0]:
        UMAX = u
        NPOINTS = int(round(u / dw))
        p, _ = heston_call(S, v_bar, tau_bar)
        g = get_greeks(S, v_bar, tau_bar)
        print(f"    UMAX={u:7.0f} N={NPOINTS:6d}  price={p:.9f}  "
              f"tconv={g['tconv']:+.0f}  veta={g['veta']:+.2f}  charm={g['charm']:+.3f}")
    UMAX, NPOINTS = u0, n0

    # 5. FD bump-size sensitivity for the finite-difference Greeks (1-day ATM).
...
def realized_eps_rho(prices, variances):
    # the paper's check on the chosen xi and rho: scale each bar's variance
    # increment by sqrt(v)sqrt(dt), which is the diffusion coefficient of the
    # Heston variance SDE with xi divided out, so the sample sd of the scaled
    # increments estimates xi directly. the price increments are scaled the same
    # way, and their correlation estimates rho.
    dt = ONE_DAY / (len(prices[0]) - 1)
    dv_scaled, dS_scaled = [], []
    for S, v in zip(prices, variances):
        for b in range(len(S) - 1):
            if v[b] <= 0:
                continue
            denom = math.sqrt(v[b]) * math.sqrt(dt)
            dv_scaled.append((v[b + 1] - v[b]) / denom)
            dS_scaled.append((S[b + 1] - S[b]) / (S[b] * denom))
    dv_scaled = np.array(dv_scaled)
    dS_scaled = np.array(dS_scaled)
    return (float(np.std(dv_scaled, ddof=1)),
            float(np.corrcoef(dS_scaled, dv_scaled)[0, 1]),
            len(dv_scaled))


def session_calendar(dates):
    # where the day count comes from: every weekday in the window, less the
    # sessions the market was closed. returns (n_weekdays, [missing dates]).
    d0 = datetime.date.fromisoformat(dates[0])
    d1 = datetime.date.fromisoformat(dates[-1])
    span = [d0 + datetime.timedelta(days=i) for i in range((d1 - d0).days + 1)]
    weekdays = [d.isoformat() for d in span if d.weekday() < 5]
    have = set(dates)
    return len(weekdays), [d for d in weekdays if d not in have]
...
def detrend_study():
    # VIX1D climbs through the session because its weight rolls off today's expiring
    # strip and onto tomorrow's, not because variance is rising. that climb hands the
    # vega term a drift by construction. remove it and see what survives.
    # NB: VIX1D measures time in BUSINESS minutes, not calendar minutes, and targets a
    # constant maturity of M_CM = 405 business minutes (one 09:30-16:15 session). Cboe
    # 1-Day Volatility Index Methodology v2.0, section 3.1 steps 3-4. Because business
    # time skips the overnight gap, tomorrow's expiry sits exactly 405 business minutes
    # behind today's, so the near-term interpolation weight collapses to M_T1 / 405.
    global THETA
    ...
    print("  average level through the session, and Cboe's near-term interpolation")
    print("  weight on the expiring strip, (390 - 5b) / 405 business minutes:")
    for b in [0, 12, 24, 39, 48, 60, 77]:
        clock = 30 + 5 * b
        weight = (390 - 5 * b) / 405.0
        frozen = "  (near-term frozen)" if clock >= 360 else ""
        print(f"    bar {b:2d} ({9 + clock // 60:02d}:{clock % 60:02d})  "
              f"{mean_path[b]:6.2f} pts   weight {100 * weight:5.1f}%{frozen}")
    ...
    for name, Vx in [("baseline", V), ("additive", additive),
        ...
        # the paper quotes the turbulent-tercile gross shares after adjustment, so
        # print them here. terciles are sorted on |net move| of the ADJUSTED series.
        by_tercile_gross = " / ".join(
            f"{gross_shares(totals, keep)['gamma']:.1f}|{gross_shares(totals, keep)['vega']:.1f}"
            for _, keep in terciles_by(moves))
        print(f"  gross gamma|vega by tercile: {by_tercile_gross}")
    ...
    print()
    print("  The roll accounts for the vega drift: removing it collapses that drift")
    print("  while leaving the attribution shares in place. It is not the only thing")
    print("  that moves -- the gamma drift roughly halves and the mean daily P&L")
    print("  crosses conventional significance (t -1.87 -> -1.99 / -1.96).")
...
def main(full=False):
    ...
    nweek, closed = session_calendar(dates)
    print(f"  day count: {nweek} weekdays in the window, {len(closed)} closed "
          f"({', '.join(closed) if closed else 'none'}) -> {ndays} trading days")
    eps_hat, rho_hat, nincr = realized_eps_rho(prices, variances)
    print(f"  realized vol-of-vol from {nincr} scaled increments: xi_hat = {eps_hat:.2f} "
          f"(assumed {XI});  rho_hat = {rho_hat:+.2f} (assumed {RHO})")
    ...
    for alpha, zcrit in [(0.05, 1.6449), (0.01, 2.3263)]:
        ...
        print(f"  alpha={alpha:<5} {int(drop.sum()):2d} jump days, {int(keep.sum()):2d} clean: "
              ...
              f"vega {g['vega']:.1f}% / {v['vega']:+.1f}%, "
              f"gross residual {residual_pct(totals, pnls, keep):.1f}%, "
              f"unexplained variance {v['residual']:+.1f}%")
        if drop.sum() >= 2:
    ...
    print(f"  the two sorts correlate {np.corrcoef(vix_moves, abs_dS)[0, 1]:+.2f} (Pearson)"
          f", {np.corrcoef(_rank(vix_moves), _rank(abs_dS))[0, 1]:+.2f} (Spearman)")

    # ---- cross-term structure: do the two big terms offset, and does anything else? ----
    # every one of these appears in the paper, so print them rather than leaving
    # them to a scratch script that is not in the repo.
    print()
    print("=== CROSS-TERM STRUCTURE ===")
    rng = np.random.default_rng(12345)
    g_t, v_t = totals['gamma'], totals['vega']
    draws = [np.corrcoef(g_t[i], v_t[i])[0, 1]
             for i in (rng.integers(0, ndays, ndays) for _ in range(10000))]
    print(f"  corr(gamma, vega) = {np.corrcoef(g_t, v_t)[0, 1]:+.3f}  "
          f"bootstrap 95% [{np.percentile(draws, 2.5):+.2f}, {np.percentile(draws, 97.5):+.2f}]")
    C = np.corrcoef(np.array([totals[n] for n in TERM_NAMES]))
    worst, pair = 1.0, None
    for i in range(len(TERM_NAMES)):
        for j in range(i + 1, len(TERM_NAMES)):
            if C[i, j] < worst:
                worst, pair = C[i, j], (TERM_NAMES[i], TERM_NAMES[j])
    ratio = np.std(totals[pair[0]]) / np.std(totals[pair[1]])
    print(f"  most negative pair: {pair[0]}/{pair[1]} = {worst:+.2f}, "
          f"sd ratio {ratio:.0f}x -- offset is immaterial in P&L terms")
    print("  by |dVIX1D| tercile, corr(vega, pnl) and the sd growth of each term:")
    tc = list(terciles_by(vix_moves))
    corrs = " / ".join(f"{np.corrcoef(totals['vega'][k], pnls[k])[0, 1]:+.2f}" for _, k in tc)
    print(f"    corr(vega, pnl): {corrs}")
    for name in ('gamma', 'vega'):
        sds = [np.std(totals[name][k]) for _, k in tc]
        print(f"    {name:>5} sd calm -> turbulent: {sds[0]:.3e} -> {sds[2]:.3e} "
              f"(growth {sds[2] / sds[0]:.2f}x)")
    ...
    print("  proxy's weight rolling onto tomorrow's strip, not of the market.")
    ...
    for name, values, setter in [("kappa", [1.0, 3.0, 5.0], "KAPPA"),
        ...
        for val in values:
            ...
            tc = terciles_by(vix_moves)
            gc = gross_shares(t_s, tc[0][1])
            gt = gross_shares(t_s, tc[2][1])
            print(f"  {name}={val:<4}: gross gamma {g['gamma']:5.1f}% vega {g['vega']:5.1f}% "
                  ...
                  f"calm->turb gross gamma {gc['gamma']:.1f}->{gt['gamma']:.1f}% "
                  f"vega {gc['vega']:.1f}->{gt['vega']:.1f}%  |  "
