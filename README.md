...
python fetch_data.py                       # appends fresh sessions to archive.csv (see Data provenance below)
```
...
Running `fetch_data.py` will not reproduce this window: Yahoo only serves 5-minute index data for the trailing 60 trading days, so a fresh pull returns whatever the most recent 60 days are at the time it's run. It therefore appends into `archive.csv`, seeded from `intraday.csv` on first run, rather than rebuilding in place — run it weekly and the sample grows instead of sliding forward. Existing rows are never overwritten, because Yahoo does revise its intraday history: 11 of the 4,446 bars in `intraday.csv` differ from what a fresh pull returns today. Promote `archive.csv` to `intraday.csv` by hand when cutting a new version. Use the committed `intraday.csv` to reproduce the paper's reported numbers exactly.

## Files

`paper1.pdf` / `paper1.tex` are the paper. `paper1_implementation.py` holds the pricer, the decomposition, and every statistic the paper reports. `make_figures.py` regenerates both figures from the same pipeline. `fetch_data.py` appends fresh sessions to `archive.csv` (see caveat above). `intraday.csv` is the committed sample, frozen at the 57 days the paper reports, with columns `timestamp`, `und_close`, `vix1d_close`. `requirements.txt` lists the Python dependencies (numpy, pandas, yfinance, matplotlib).

## Results and caveats

Two measures are reported: the variance share, a signed decomposition of the variance of the realized hedged P&L (Gamma 64.6%, Vega 31.6%, 5.6% unexplained), and the gross share, each term's average absolute daily contribution (Gamma 50.4%, Vega 35.0%, on a 20.1% gross residual). Vega's gross share roughly doubles from the calmest days to the most turbulent. The Heston parameters are uncalibrated round numbers apart from eta, so read this as a proof of concept rather than a calibrated estimate. VIX1D climbs mechanically through the session, averaging 10.9 at the open and 13.9 at the close, because it measures time in business minutes against a constant maturity of 405 of them while Cboe's near-term weight on today's expiring strip falls from 96.3% to 1.2% across the day. That roll manufactures the drift in the Vega term; Section 4.3 removes it and reruns, and the drift collapses from t = +6.5 to +0.6 while the gross shares barely move. Vega's variance share is the exception, rising from 31.6% to 37.2% once the spurious drift is out.
